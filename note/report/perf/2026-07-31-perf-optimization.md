...
| 场景 | 优化前 | 优化后 | 加速 |
|---|---|---|---|
| 不匹配·稳态（idle 常态） | 195.4 | 25.7 | **7.59×** |
| 不匹配·冷启动（输入刚变） | 195.4 | 261.4 | 0.75×（见注） |
| 命中·稳态 | 392.1 | 42.8 | **9.16×** |
| 空输入·稳态 | 268.6 | 12.8 | **20.98×** |

> 注：冷启动路径新旧算法工作量相同（都是全量扫描），优化收益**来自稳态跳过重匹配**，
> 而非单次匹配更快。这是该优化的本质：idle 机器不再每 tick 重复劳动。
>
> **签名按值比较**：`BlockMenu.getItemInSlot` 经 Bukkit `Inventory.getItem` 返回，Craftbukkit
> 通常每次返回**新的 `CraftItemStack` 包装对象**（引用不等、底层 NMS 物品相同）。因此签名
> 缓存用 `isSimilar` + 数量**按值**判定“输入是否变化”（而非引用相等）——否则缓存永不命中。
> 按值比较仅需输入槽数量次 isSimilar（1~3 次），仍远少于全量扫描的 ~315 次。
...
| 维度 | 优化前 | 优化后 | 加速 |
|---|---|---|---|
| fits（4 槽，clone → int） | 76.8 | 28.0 | **2.74×** |
| berry 查表（~60 项，线性 → HashMap） | 151.9 | 2.5 | **60.39×** |
| subRecipes（35 项，重建 → 缓存） | 128.2 | 27.0 | **4.75×** |
